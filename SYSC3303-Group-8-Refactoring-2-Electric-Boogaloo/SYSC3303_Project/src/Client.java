...
public class Client {
	...
	public Client() {
		...
		while (true) {
			String IPAddress = NetworkConfig.IPSelector();
			...
			try {
				...
				System.err.println(Globals.getErrorMessage("Client", "cannot get the IP address"));
				e.printStackTrace();
				//System.exit(-1);
				continue;
			}
	...
	//This function selects the port after the use chooses his mode.
	public void modeSelector() {
		...
		while (true) {
			...
			int userInput = choiceScanner.nextInt();
	 ...
	 *            packet type
	 * @param fileName
	 *            name of the file that is requested to be read or written (in
	 *            bytes)
	 * @param mode
	 *            mode (in bytes)
	 * @param ipAddress
	 ...
	 *            server port
	 * 
	 * @Return Datagram packet received from the server after making a RRQ or WRQ
	 *         request
	 */
	private void makeReadWriteRequest(TFTPPacketType packetType, String fileName, String mode, InetAddress ipAddress,
			int port) {
		...
		if (packetType == TFTPPacketType.RRQ) {
			...
			requestPacket = TFTPPacketBuilder.getRRQWRQDatagramPacket(TFTPPacketType.RRQ, fileName, mode, ipAddress,
					port);
		} else {
			// get write request packet
			requestPacket = TFTPPacketBuilder.getRRQWRQDatagramPacket(TFTPPacketType.WRQ, fileName, mode, ipAddress,
					port);
		}
	 ...
	 * 
	 * @param filePath
	 *            path of the file that the client requests
	 * @param mode
	 *            mode of request
	 */
	public void readFile(String filePath, String mode) {
		...
		//try {
			makeReadWriteRequest(TFTPPacketType.RRQ, fileName, mode, serverAddress,
					serverPort);
	/*	} throw (UnknownHostException e) {
		...
		while (fileDataLen == NetworkConfig.DATAGRAM_PACKET_MAX_LEN) {
			...
			if (dataPacket.getBlockNumber() == 1) {
				...
				if (fmRes.error) {
					// access violation error will send an error packet with error code 2 and the
					// connection
					if (fmRes.accessViolation)
						errorHandler.sendAccessViolationErrorPacket(
								String.format("write access denied to file: %s", fileName), serverAddress, serverPort);
					// disk full error will send an error packet with error code 3 and close the
					// connection
					else if (fmRes.fileAlreadyExist)
						errorHandler.sendFileExistsErrorPacket(String.format("file already exists: %s", fileName),
								serverAddress, serverPort);
					else if (fmRes.diskFull)
						errorHandler.sendDiskFullErrorPacket(
								String.format("Not enough disk space for file: %s", fileName), serverAddress,
								serverPort);
					return;
				}
			...
			if (fmRes.error) {
				...
				// connection
					...
					errorHandler.sendDiskFullErrorPacket(String.format("Not enough disk space for file: %s", fileName),
							serverAddress, serverPort);
				return;
			}
	 ...
	 * 
	 * @param filePath
	 *            path of the file client wants to write to
	 * @param mode
	 *            mode of the request
	 */
	public void writeFile(String filePath, String mode) {
		...
		//try { //CHANGE THE PROXY PORT
			makeReadWriteRequest(TFTPPacketType.WRQ, fileName, mode, serverAddress,
					serverPort);
/*		} catch (UnknownHostException e) {
			System.err.println(Globals.getErrorMessage("Client", "cannot get localhost address"));
			e.printStackTrace();
		...
		if (ackPacket.getBlockNumber() == 0) {
			...
			if (!res.error) {
				...
				if (res.accessViolation)
					errorHandler.sendAccessViolationErrorPacket(
							String.format("read access denied to file: %s", fileName), serverAddress, serverPort);
				// file not found error will send an error packet with error code 1 and the
				// connection
				else if (res.fileNotFound)
					errorHandler.sendFileNotFoundErrorPacket(String.format("file not found: %s", fileName),
							serverAddress, serverPort);

				return;
			}

			// create list of DATA datagram packets that contain up to 512 bytes of file
			// data
			Queue<DATAPacket> dataPacketStack = TFTPPacketBuilder.getStackOfDATADatagramPackets(fileData, serverAddress,
					serverPort);

			DATAPacket dataPacket = null;
			while (!dataPacketStack.isEmpty()) {
				// send each datagram packet in order and wait for acknowledgement packet from
				// the client
