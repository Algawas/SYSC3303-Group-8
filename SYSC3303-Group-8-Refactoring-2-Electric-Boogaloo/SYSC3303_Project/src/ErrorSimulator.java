	...
	public ErrorSimulator() {
		while (true) {
			...
			try {
				...
				System.err.println(Globals.getErrorMessage("Error Simulator", "cannot get the Server IP address"));
				e.printStackTrace();
				//System.exit(-1);
				continue;
			}
	...
	public void shutdown() {
		...
		if (!tftpSocket.isClosed()) {
			...
			try {
				...
				shutdownClient
						.send(new DatagramPacket(new byte[0], 0, InetAddress.getByName("localhost"), NetworkConfig.PROXY_PORT));
